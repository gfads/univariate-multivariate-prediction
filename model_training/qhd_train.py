...
df = read_csv(ROOT_DIR + '/database/qhd/' + forecasting_time + '/train.csv')

data_list = ['pods', 'cpu', 'heap', 'jvm_total_MarkSweepCompact', 'jvm_total_Copy', 'jvm_seconds_Copy',
             'jvm_seconds_MarkSweepCompact', 'memory', 'rt', 'tp']
...
if not path.isdir(ROOT_DIR + '/knowledge/models/qhd/' + forecasting_time + '/'):
    makedirs(ROOT_DIR + '/knowledge/models/qhd/' + forecasting_time + '/')

pickle.dump(scaler, open(ROOT_DIR + '/knowledge/models/qhd/' + forecasting_time + '/scaler.pkl', 'wb'))
...
for b, e, h, l, u in hyper_param:
    ...
    if not isnan(prediction).any():
        ...
        if accurracy < bm:
            print('MSE: ', accurracy)
            bm = accurracy
            model.save(ROOT_DIR + '/knowledge/models/qhd/' + forecasting_time + '/model.h5')
