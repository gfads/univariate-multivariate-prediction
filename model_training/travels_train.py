...
for service in ['cars', 'discounts', 'flights', 'hotels', 'insurances', 'travels']:
    bm = Inf

    df = read_csv(ROOT_DIR + '/database/travels/' + service + '/' + forecasting_time + '/train.csv')
    ...
    if not path.isdir(ROOT_DIR + '/knowledge/models/travels/' + service + '/' + forecasting_time + '/'):
        makedirs(ROOT_DIR + '/knowledge/models/travels/' + service + '/' + forecasting_time + '/')

    pickle.dump(scaler, open(ROOT_DIR + '/knowledge/models/travels/' + service + '/' + forecasting_time + '/scaler.pkl', 'wb'))
    ...
    for b, e, h, l, u in hyper_param:
        ...
        if not isnan(prediction).any():
            ...
            if accurracy < bm:
                print('MSE: ', accurracy)
                bm = accurracy
                model.save(ROOT_DIR + '/knowledge/models/travels/' + service + '/' + forecasting_time + '/model.h5')
